...
export function Modal({ isOpen, onClose, title, children, maxWidth = "max-w-lg" }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <div className={`relative bg-white dark:bg-surface-800 rounded-xl shadow-xl w-full ${maxWidth} max-h-[90vh] overflow-hidden animate-in-fast`}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-surface-100 dark:border-surface-700/60">
          ...
          <button
            onClick={onClose}
            className="text-surface-400 hover:text-surface-600 dark:hover:text-surface-200 p-2 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-700/50 transition-colors"
