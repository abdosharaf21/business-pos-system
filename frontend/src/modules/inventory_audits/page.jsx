...
import { cardClassOverflowHidden, filterBarClass, inputClass, selectClass, labelClass } from "../../shared/components/styles";
      ...
      <div className={filterBarClass + " mb-6"}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
          <div className="relative">
            ...
            <input type="text" placeholder={t("inventoryAudits.searchPlaceholder")} value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="w-full ps-10 pe-3.5 py-2.5 border border-surface-200 dark:border-surface-700/60 bg-surface-50 dark:bg-surface-700/40 rounded-xl text-sm text-surface-800 dark:text-surface-100 placeholder:text-surface-300 dark:placeholder:text-surface-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all duration-150" aria-label={t("inventoryAudits.searchPlaceholder")} />
          </div>
          <div>
          ...
          <div className="flex items-center gap-2">
            <select value={sort} onChange={(e) => { setSort(e.target.value); setPage(1); }} className="flex-1 px-3 py-1.5 rounded-lg border border-surface-200 dark:border-surface-700/60 text-[12px] text-surface-700 dark:text-surface-200 bg-surface-50 dark:bg-surface-700/40 focus:outline-none focus:ring-2 focus:ring-primary-500/20" aria-label={t("inventoryAudits.sortBy")}>
              {SORT_COLUMNS.map((col) => (
                <option key={col.key} value={col.key}>{t(col.labelKey)}</option>
              ))}
            </select>
            <button onClick={() => { setOrder((prev) => (prev === "asc" ? "desc" : "asc")); setPage(1); }} className="px-3 py-1.5 rounded-lg border border-surface-200 dark:border-surface-700/60 text-[12px] font-semibold text-surface-600 dark:text-surface-300 bg-surface-50 dark:bg-surface-700/40 hover:bg-surface-100 dark:hover:bg-surface-700/60 transition-colors" title={order === "asc" ? t("inventoryAudits.sortAsc") : t("inventoryAudits.sortDesc")}>
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-surface-100 dark:border-surface-700/60">
          ...
          {hasFilters && (
            <button onClick={clearFilters} className="flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-semibold text-surface-500 dark:text-surface-400 bg-surface-50 dark:bg-surface-700/40 hover:bg-surface-100 dark:hover:bg-surface-700/60 rounded-lg transition-colors">
        ...
        <div className={cardClassOverflowHidden}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm" role="table">
              <thead>
                <tr className="border-b border-surface-100 dark:border-surface-700/60 bg-surface-50/60 dark:bg-surface-700/40">
                  {[
                    { key: "name", labelKey: "inventoryAudits.columns.name" },
                    { key: "location", labelKey: "inventoryAudits.columns.location" },
                    { key: "status", labelKey: "inventoryAudits.columns.status" },
                  ...
                  <th className="px-5 py-3.5 text-end text-[11px] font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wider">{t("inventoryAudits.columns.actions")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-surface-100 dark:divide-surface-700/60">
                {items.map((audit) => (
                  <tr key={audit.id} className="hover:bg-surface-50/50 dark:hover:bg-surface-700/30 transition-colors duration-100">
                    <td className="px-5 py-3.5">
                      <button onClick={() => setDetails(audit)} className="text-[13px] font-semibold text-surface-800 dark:text-surface-100 hover:text-primary-600 dark:hover:text-primary-400 text-start transition-colors">
                        {audit.name}
                      </button>
                    </td>
                    <td className="px-5 py-3.5 whitespace-nowrap text-[13px] text-surface-600 dark:text-surface-300">{t(`inventoryAudits.locations.${audit.location}`)}</td>
                    ...
                    <td className="px-5 py-3.5 whitespace-nowrap">
                      {audit.total_difference !== 0 ? (
                        <span className={`text-[13px] font-bold tabular-nums whitespace-nowrap ${audit.total_difference > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>
                          {audit.total_difference > 0 ? "+" : ""}{audit.total_difference}
                        </span>
                      ) : (
                        <span className="text-[13px] text-surface-400 dark:text-surface-500">0</span>
                      )}
                    </td>
                    <td className="px-5 py-3.5 whitespace-nowrap text-[13px] text-surface-500 dark:text-surface-400">{audit.created_by_name || "—"}</td>
                    ...
                    <td className="px-5 py-3.5 whitespace-nowrap text-end">
                      <div className="inline-flex items-center gap-1">
                        <button onClick={() => setDetails(audit)} className="p-1.5 text-surface-400 dark:text-surface-500 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-500/10 rounded-lg transition-all duration-150" title={t("inventoryAudits.details.title")}>
                          <Eye className="w-4 h-4" />
                        </button>
                        {canManage && audit.status === "open" && (
                          <>
                            <button onClick={() => setCounting(audit)} className="p-1.5 text-surface-400 dark:text-surface-500 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-500/10 rounded-lg transition-all duration-150" title={t("inventoryAudits.form.countTitle")}>
                              <Pencil className="w-4 h-4" />
                            </button>
                            <button onClick={() => setCompleteTarget(audit)} className="p-1.5 text-surface-400 dark:text-surface-500 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-lg transition-all duration-150" title={t("inventoryAudits.complete")}>
                              <CheckCircle2 className="w-4 h-4" />
                            </button>
                            <button onClick={() => setCancelTarget(audit)} className="p-1.5 text-surface-400 dark:text-surface-500 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-500/10 rounded-lg transition-all duration-150" title={t("inventoryAudits.cancel")}>
                              <Ban className="w-4 h-4" />
                            </button>
                            <button onClick={() => setDeleteTarget(audit)} className="p-1.5 text-surface-400 dark:text-surface-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-all duration-150" title={t("inventoryAudits.confirmDelete.title")}>
          ...
          {pages > 1 && (
            <div className="flex items-center justify-between px-5 py-3.5 border-t border-surface-100 dark:border-surface-700/60">
              <p className="text-[12px] text-surface-400 dark:text-surface-500">{t("inventoryAudits.pageOf", { page, pages })}</p>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setPage((p) => Math.max(1, p - 1))}
                  disabled={page <= 1}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-surface-200 dark:border-surface-700/60 text-[12px] font-semibold text-surface-600 dark:text-surface-300 hover:bg-surface-50 dark:hover:bg-surface-700/40 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={t("inventoryAudits.form.title")}>
      <form onSubmit={handleSubmit((data) => { onSubmit(data); reset(); })} className="space-y-5">
        ...
        <div>
          ...
          {errors.location && <p className="text-[11px] text-red-500 dark:text-red-400 mt-1 font-medium">{errors.location.message}</p>}
        </div>
        <div className="bg-surface-50 dark:bg-surface-700/40 rounded-xl p-4 text-[13px] text-surface-600 dark:text-surface-300 leading-relaxed">
        ...
        <div className="flex justify-end gap-3 pt-5 border-t border-surface-100 dark:border-surface-700/60">
          <button type="button" onClick={onClose} className="px-4 py-2.5 text-[13px] font-semibold text-surface-600 dark:text-surface-300 bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700/60 rounded-xl hover:bg-surface-50 dark:hover:bg-surface-700 transition-all duration-150">{t("inventoryAudits.form.cancel")}</button>
    ...
    <Modal isOpen={!!audit} onClose={onClose} title={t("inventoryAudits.form.countTitle")} maxWidth="max-w-3xl">
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <span className="text-[12px] font-semibold text-surface-400 dark:text-surface-500 uppercase tracking-wide">{t("inventoryAudits.columns.name")}</span>
            <span className="text-[13px] font-bold text-surface-800 dark:text-surface-100">{audit.name}</span>
          </div>
          ...
          <span className="w-px h-4 bg-surface-200 dark:bg-surface-700/60" />
          <span className="text-[12px] font-semibold text-surface-400 dark:text-surface-500">
        ...
        {itemsLoading ? (
          ...
          <p className="text-center text-surface-400 dark:text-surface-500 py-10 text-[13px]">{t("inventoryAudits.noItems")}</p>
        ) : (
          <div className="max-h-[50vh] overflow-y-auto border border-surface-100 dark:border-surface-700/60 rounded-xl">
            <table className="w-full text-sm" role="table">
              <thead className="sticky top-0 bg-surface-50 dark:bg-surface-700/40">
                <tr className="border-b border-surface-100 dark:border-surface-700/60">
                  <th className="px-4 py-3 text-start text-[11px] font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wider">{t("inventoryAudits.columns.product")}</th>
                  <th className="px-4 py-3 text-center text-[11px] font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wider">{t("inventoryAudits.columns.system")}</th>
                  <th className="px-4 py-3 text-center text-[11px] font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wider">{t("inventoryAudits.columns.counted")}</th>
                  <th className="px-4 py-3 text-center text-[11px] font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wider">{t("inventoryAudits.columns.difference")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-surface-100 dark:divide-surface-700/60">
                {rows.map((row) => (
                  <tr key={row.product_id} className="hover:bg-surface-50/50 dark:hover:bg-surface-700/30">
                    <td className="px-4 py-2.5">
                      <p className="text-[13px] font-semibold text-surface-800 dark:text-surface-100">{row.product_name}</p>
                      {row.barcode && <p className="text-[11px] text-surface-400 dark:text-surface-500">{row.barcode}</p>}
                    </td>
                    <td className="px-4 py-2.5 text-center text-[13px] text-surface-600 dark:text-surface-300 tabular-nums whitespace-nowrap">{row.system_quantity}</td>
                    <td className="px-4 py-2.5 text-center">
                      <input
                        ...
                        className="numeric-grow min-w-24 px-3 py-1.5 text-center border border-surface-200 dark:border-surface-700/60 bg-surface-50 dark:bg-surface-700/40 rounded-lg text-sm text-surface-800 dark:text-surface-100 placeholder:text-surface-300 dark:placeholder:text-surface-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all duration-150"
                      />
                    </td>
                    <td className="px-4 py-2.5 text-center">
                      {row.counted_quantity !== null && row.difference !== 0 ? (
                        <span className={`text-[13px] font-bold tabular-nums whitespace-nowrap ${row.difference > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>
                          {row.difference > 0 ? "+" : ""}{row.difference}
                        </span>
                      ) : (
                        <span className="text-[13px] text-surface-300 dark:text-surface-600">0</span>
                      )}
                    </td>
                  </tr>
                ))}
        ...
        <p className="text-[12px] text-surface-400 dark:text-surface-500 leading-relaxed">{t("inventoryAudits.form.countHint")}</p>
    ...
    <Modal isOpen={!!audit} onClose={onClose} title={t("inventoryAudits.details.title")} maxWidth="max-w-3xl">
      <div className="space-y-5">
        <div className="bg-surface-50 dark:bg-surface-700/40 rounded-xl p-4">
          <p className="text-[11px] font-semibold text-surface-400 dark:text-surface-500 uppercase tracking-wide mb-1">{t("inventoryAudits.columns.name")}</p>
          <p className="text-[15px] font-bold text-surface-900 dark:text-surface-100">{audit.name}</p>
        </div>
        ...
        <div>
          <h3 className="text-[15px] font-semibold text-surface-900 dark:text-surface-100 mb-3">{t("inventoryAudits.details.items")}</h3>
          {itemsLoading ? (
            <LoadingSpinner />
          ) : itemsError ? (
            <ErrorDisplay message={itemsError} />
          ) : items.length === 0 ? (
            <p className="text-center text-surface-400 dark:text-surface-500 py-8 text-[13px]">{t("inventoryAudits.noItems")}</p>
          ) : (
            <div className="max-h-[40vh] overflow-y-auto border border-surface-100 dark:border-surface-700/60 rounded-xl">
              <table className="w-full text-sm" role="table">
                <thead className="sticky top-0 bg-surface-50 dark:bg-surface-700/40">
                  <tr className="border-b border-surface-100 dark:border-surface-700/60">
                    <th className="px-4 py-3 text-start text-[11px] font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wider">{t("inventoryAudits.columns.product")}</th>
                ...
                <tbody className="divide-y divide-surface-100 dark:divide-surface-700/60">
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-surface-50/50 dark:hover:bg-surface-700/30">
                      <td className="px-4 py-2.5">
                        <p className="text-[13px] font-semibold text-surface-800 dark:text-surface-100">{item.product_name}</p>
                        {item.barcode && <p className="text-[11px] text-surface-400 dark:text-surface-500">{item.barcode}</p>}
                      </td>
                      <td className="px-4 py-2.5 text-center text-[13px] text-surface-600 dark:text-surface-300 tabular-nums whitespace-nowrap">{item.system_quantity}</td>
                      <td className="px-4 py-2.5 text-center text-[13px] text-surface-600 dark:text-surface-300 tabular-nums whitespace-nowrap">
                        {item.counted_quantity === null ? "—" : item.counted_quantity}
                      </td>
                      <td className="px-4 py-2.5 text-center">
                        {item.difference !== 0 ? (
                          <span className={`text-[13px] font-bold tabular-nums whitespace-nowrap ${item.difference > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>
                            {item.difference > 0 ? "+" : ""}{item.difference}
                          </span>
                        ) : (
                          <span className="text-[13px] text-surface-300 dark:text-surface-600 tabular-nums">0</span>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-surface-100 dark:border-surface-700/60">
          ...
          <button onClick={onClose} className="px-4 py-2.5 text-[13px] font-semibold text-surface-600 dark:text-surface-300 bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700/60 rounded-xl hover:bg-surface-50 dark:hover:bg-surface-700 transition-all duration-150">{t("inventoryAudits.details.close")}</button>
        </div>
      </div>
    </Modal>
  );
