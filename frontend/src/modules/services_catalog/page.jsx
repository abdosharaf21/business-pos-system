...
function ServiceCard({ service, t, navigate }) {
  ...
  const colors = COLOR_MAP[service.color] || COLOR_MAP.default;
  const statusConf = STATUS_CONFIG[service.status] || STATUS_CONFIG.active;
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[13px] text-surface-500 dark:text-surface-400 mt-1 leading-relaxed">
            {t(service.descriptionKey, service.descriptionFallback)}
          </p>
        </div>
      </div>
...
export default function ServicesCatalogPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("servicesCatalog.title", "Services Catalog")}
        description={t("servicesCatalog.description", "Select a service to open its dedicated workspace")}
      />

      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {SERVICE_APPS.map((service) => (
          <ServiceCard key={service.id} service={service} t={t} navigate={navigate} />
        ))}
      </div>
    </div>
  );
