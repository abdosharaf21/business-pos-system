...
export default function PurchasesPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Status",
      render: (val) => {
        const variants = { completed: "success", pending: "warning", cancelled: "inactive" };
        return (
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          <button onClick={() => setViewPurchase(row)} className="p-2 text-surface-400 hover:text-primary-600 hover:bg-primary-50 rounded-xl transition-all duration-150" title="View Details">
            <Eye className="w-4 h-4" />
          </button>
          <button onClick={() => handleViewInvoice(row)} className="p-2 text-surface-400 hover:text-primary-600 hover:bg-primary-50 rounded-xl transition-all duration-150" title="Invoice">
  ...
  return (
    <div>
      <PageHeader
        title="Purchases"
        description="Manage purchase orders and inventory replenishment"
        actions={
      ...
      {!showCreateForm && (
        <>
          <div className="mb-6 relative">
            ...
            <input type="text" placeholder="Search by invoice number or supplier..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-surface-200 bg-white rounded-xl text-sm text-surface-800 placeholder:text-surface-300 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all duration-150 shadow-card" />
          </div>

          {filtered.length === 0 ? (
            <EmptyState icon={ShoppingCart} title="No purchases found" description={search ? "Try a different search" : "Create your first purchase"} action={!search && canManage && <button onClick={() => setShowCreateForm(true)} className="px-4 py-2.5 bg-gradient-to-r from-primary-600 to-primary-700 text-white text-[13px] font-semibold rounded-xl hover:from-primary-700 hover:to-primary-800 shadow-sm shadow-primary-600/20">New Purchase</button>} />
          ) : (
...
function CreatePurchaseForm({ onDone, onCancel }) {
  ...
  const handleSubmit = async () => {
    ...
    for (const item of items) {
      if (item.quantity <= 0) { toast.error("Quantity must be greater than zero"); return; }
      if (item.cost_price < 0) { toast.error("Cost price cannot be negative"); return; }
    }
    setSubmitting(true);
    try {
  ...
  return (
    <div className="bg-white border border-surface-200 rounded-2xl p-6 mb-6 shadow-card">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-[15px] font-bold text-surface-900">New Purchase Order</h2>
      ...
      {showAddSupplier && (
        <div className="mb-5 p-4 border border-primary-200 bg-primary-50/40 rounded-xl">
          <h3 className="text-[14px] font-bold text-surface-900 mb-4">New Supplier</h3>
          <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <div>
              <label className="block text-[12px] font-semibold text-surface-600 mb-1">Phone *</label>
              <input
                type="text"
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <div>
              <label className="block text-[12px] font-semibold text-surface-600 mb-1">Address</label>
              <input
                type="text"
      ...
      <div className="mb-5" ref={searchRef}>
        <label className={LABEL_CLASS}>Add Products</label>
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-surface-200 bg-white rounded-xl text-sm text-surface-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all duration-150"
          />
        </div>
        {showResults && (
          <div className="mt-2 border border-surface-200 rounded-xl overflow-hidden bg-white shadow-card max-h-64 overflow-y-auto">
            {searching ? (
              <div className="px-4 py-3 text-[13px] text-surface-400 text-center">Searching...</div>
            ) : searchResults.length === 0 ? (
              <div className="px-4 py-3 text-[13px] text-surface-400 text-center">No products found</div>
            ) : (
              searchResults.map((p) => (
                <button key={p.id} onClick={() => addItem(p)} className="flex items-center justify-between w-full px-4 py-2.5 hover:bg-surface-50 text-left transition-colors border-b border-surface-100 last:border-b-0">
                  <div className="min-w-0 flex-1">
                    <span className="text-[13px] font-semibold text-surface-800">{p.name}</span>
                    <div className="flex gap-2 mt-0.5">
                      {p.sku && <span className="text-[11px] font-mono text-surface-400">SKU: {p.sku}</span>}
                      {p.barcode && <span className="text-[11px] font-mono text-surface-400">({p.barcode})</span>}
                    </div>
                  </div>
                  <span className="text-[12px] text-surface-500 ml-3 whitespace-nowrap">${parseFloat(p.purchase_price).toFixed(2)}</span>
                </button>
              ))
            )}
          </div>
        ...
        <div className="mb-5">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-surface-200">
                  <th className="text-left py-2.5 px-3 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">Product</th>
                  <th className="text-center py-2.5 px-3 text-[12px] font-semibold text-surface-500 uppercase tracking-wider w-[100px]">Qty</th>
                  <th className="text-center py-2.5 px-3 text-[12px] font-semibold text-surface-500 uppercase tracking-wider w-[120px]">Cost Price</th>
                  <th className="text-right py-2.5 px-3 text-[12px] font-semibold text-surface-500 uppercase tracking-wider w-[100px]">Subtotal</th>
              ...
              <tbody>
                {items.map((item, i) => (
                  <tr key={i} className="border-b border-surface-100">
                    ...
                    <td className="py-2.5 px-3 text-right text-[13px] font-semibold text-surface-800">${(item.quantity * item.cost_price).toFixed(2)}</td>
                    <td className="py-2.5 px-3">
          ...
          <div className="flex justify-end mt-4 pt-4 border-t border-surface-200">
            <div className="text-right">
              <span className="text-[12px] text-surface-500 font-medium">Total</span>
              <p className="text-xl font-bold text-surface-900">${subtotal.toFixed(2)}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-end gap-3 pt-5 border-t border-surface-100">
        <button onClick={onCancel} className="px-4 py-2.5 text-[13px] font-semibold text-surface-600 bg-white border border-surface-200 rounded-xl hover:bg-surface-50 transition-all duration-150">Cancel</button>
        <button onClick={handleSubmit} disabled={submitting || items.length === 0} className="px-6 py-2.5 text-[13px] font-semibold text-white bg-gradient-to-r from-primary-600 to-primary-700 rounded-xl hover:from-primary-700 hover:to-primary-800 disabled:opacity-50 transition-all duration-150 shadow-sm shadow-primary-600/20">
          {submitting ? "Creating..." : "Complete Purchase"}
        </button>
      </div>
    </div>
  );
...
function PurchaseDetailModal({ purchase, onClose }) {
  ...
  return (
    <Modal isOpen={!!purchase} onClose={onClose} title={`Purchase #${details?.invoice_number || ""}`} size="lg">
      {isLoading ? <LoadingSpinner /> : details ? (
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-[12px] font-semibold text-surface-400 uppercase tracking-wider">Supplier</span>
              <p className="text-[14px] font-semibold text-surface-800 mt-0.5">{details.supplier_name || "-"}</p>
            </div>
            <div className="text-right">
              <span className="text-[12px] font-semibold text-surface-400 uppercase tracking-wider">Date</span>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-surface-200">
                  <th className="text-left py-2.5 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">Product</th>
                  <th className="text-center py-2.5 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">Qty</th>
                  <th className="text-right py-2.5 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">Cost</th>
                  <th className="text-right py-2.5 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">Subtotal</th>
                </tr>
              </thead>
              <tbody>
                {details.items?.map((item, i) => (
                  <tr key={i} className="border-b border-surface-100">
                    ...
                    <td className="py-2.5 text-right text-[13px] text-surface-600">${Number(item.cost_price).toFixed(2)}</td>
                    <td className="py-2.5 text-right text-[13px] font-semibold text-surface-800">${Number(item.subtotal).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex justify-end pt-3 border-t border-surface-200">
            <div className="text-right">
              <span className="text-[12px] text-surface-500 font-medium">Total Amount</span>
              <p className="text-xl font-bold text-surface-900">${Number(details.total_amount).toFixed(2)}</p>
            </div>
          </div>
        </div>
      ) : null}
...
function InvoiceModal({ invoice, onClose }) {
  ...
  return (
    <Modal isOpen={!!invoice} onClose={onClose} title={`Invoice #${invoice.invoice_number}`} size="lg">
      <div className="space-y-5">
        <div className="text-center pb-4 border-b border-surface-200">
          <h2 className="text-lg font-bold text-surface-900">PURCHASE INVOICE</h2>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div className="text-right">
            ...
            <span className="text-[12px] font-semibold text-surface-400 uppercase tracking-wider block mt-3">Processed By</span>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-200">
                <th className="text-left py-2.5 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">#</th>
                <th className="text-left py-2.5 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">Product</th>
                <th className="text-center py-2.5 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">Barcode</th>
                <th className="text-center py-2.5 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">Qty</th>
                <th className="text-right py-2.5 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">Price</th>
                <th className="text-right py-2.5 text-[12px] font-semibold text-surface-500 uppercase tracking-wider">Total</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className="flex justify-end pt-3 border-t border-surface-200">
          <div className="text-right">
            <span className="text-[12px] text-surface-500 font-medium">Total Amount</span>
            <p className="text-2xl font-bold text-surface-900">${Number(invoice.total_amount).toFixed(2)}</p>
          </div>
        </div>

        <div className="text-center pt-4 border-t border-surface-200">
          <p className="text-[11px] text-surface-400">Invoice generated automatically by POS System</p>
        </div>
      </div>
    </Modal>
  );
