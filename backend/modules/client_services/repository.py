...
class ClientServiceRepository:
    ...
    def get_by_client(self, client_id: int) -> List[ClientService]:
        ...
        with self._database.connection() as conn, db_cursor(conn) as cursor:
            try:
                query = (
                    f"SELECT {self._SELECT_COLUMNS} FROM client_services "
    ...
    def get_by_service(self, service_id: int) -> List[ClientService]:
        ...
        with self._database.connection() as conn, db_cursor(conn) as cursor:
            try:
                query = (
                    f"SELECT {self._SELECT_COLUMNS} FROM client_services "
                    "WHERE service_id = %s ORDER BY created_at DESC"
                )
        ...
        with self._database.connection() as conn, db_cursor(conn) as cursor:
            try:
                ...
                if cursor.rowcount > 0:
                    cursor.execute(
                        f"SELECT {self._SELECT_COLUMNS} FROM client_services WHERE id = %s",
                        (client_service.id,)
                    )
                    row = cursor.fetchone()
                    if row:
